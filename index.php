...
function markdown_autoload($args = array())
{
	...
	if ( ($options['markdown_level'] == 1) or ($options['markdown_level'] == 3) ) mso_hook_add( 'content_init', 'markdown_custom', 20); # хук на вывод контента
	if ( ($options['markdown_level'] == 2) or ($options['markdown_level'] == 3) ) mso_hook_add( 'comments_content', 'markdown_custom', 20);
	
//	mso_hook_add( 'editor_content', 'markdown_content'); // обработка текста для визуалього редактора
